...
/* Which of the following two methods of finding the Abs values is      */
...
float floatrand()
{
  ...
  for (i = 512, expo = 2.0f; i <= 16384; i *= 2, expo = expo * expo) {
    if (c & i) {
      result *= expo;
    }
  }
...
float narrowfloatrand()
{
  ...
  c = getRandom();
  result = (float) ((a - 1073741824) >> 6);
  for (i = 512, expo = 2.0f; i <= 2048; i *= 2, expo = expo * expo) {
    if (c & i) {
      result *= expo;
    }
  }
...
void exactinit(void)
{
  ...
  do {
    ...
    check = 1.0 + epsilon;
	         ...
	         (check != lastcheck));
	epsilon *= 256.0; // minor byte in mantissa assumed to be unstable
  splitter += 1.0;
...
int 
grow_expansion(int elen, REAL *e, REAL b, REAL *h)  /* e and h can be the same. */
{
  REAL Q;
  INEXACT REAL Qnew;
  int eindex;
...
int 
grow_expansion_zeroelim(int elen, REAL *e, REAL b, REAL *h)     /* e and h can be the same. */
{
  REAL Q, hh;
  INEXACT REAL Qnew;
  int eindex, hindex;
...
int 
fast_expansion_sum(int elen, REAL *e, int flen, REAL *f, REAL *h)   /* h cannot be e or f. */
{
  REAL Q;
  INEXACT REAL Qnew;
  INEXACT REAL bvirt;
...
int 
linear_expansion_sum(int elen, REAL *e, int flen, REAL *f, REAL *h)         /* h cannot be e or f. */
{
  REAL Q, q;
  INEXACT REAL Qnew;
  INEXACT REAL R;
...
int 
linear_expansion_sum_zeroelim(int elen, REAL *e, int flen, REAL *f, REAL *h)/* h cannot be e or f. */
{
  REAL Q, q, hh;
  INEXACT REAL Qnew;
  INEXACT REAL R;
...
REAL 
orient3d(REAL *pa, REAL *pb, REAL *pc)
{
  REAL bdxcdy = pb[0] * pc[1];
  REAL cdxbdy = pc[0] * pb[1];

  REAL cdxady = pc[0] * pa[1];
  REAL adxcdy = pa[0] * pc[1];

  REAL adxbdy = pa[0] * pb[1];
  REAL bdxady = pb[0] * pa[1];

  REAL det = pa[2] * (bdxcdy - cdxbdy) 
           + pb[2] * (cdxady - adxcdy)
           + pc[2] * (adxbdy - bdxady);

  REAL permanent = (Abs(bdxcdy) + Abs(cdxbdy)) * Abs(pa[2])
            + (Abs(cdxady) + Abs(adxcdy)) * Abs(pb[2])
            + (Abs(adxbdy) + Abs(bdxady)) * Abs(pc[2]);
...
// assuming px[3] are squared

REAL 
insphere(REAL *pa, REAL *pb, REAL *pc, REAL *pd)
{
  
  REAL aexbey = pa[0] * pb[1];
  REAL bexaey = pb[0] * pa[1];
  REAL ab = aexbey - bexaey;
  REAL bexcey = pb[0] * pc[1];
  REAL cexbey = pc[0] * pb[1];
  REAL bc = bexcey - cexbey;
  REAL cexdey = pc[0] * pd[1];
  REAL dexcey = pd[0] * pc[1];
  REAL cd = cexdey - dexcey;
  REAL dexaey = pd[0] * pa[1];
  REAL aexdey = pa[0] * pd[1];
  REAL da = dexaey - aexdey;

  REAL aexcey = pa[0] * pc[1];
  REAL cexaey = pc[0] * pa[1];
  REAL ac = aexcey - cexaey;
  REAL bexdey = pb[0] * pd[1];
  REAL dexbey = pd[0] * pb[1];
  REAL bd = bexdey - dexbey;

  REAL abc = pa[2] * bc - pb[2] * ac + pc[2] * ab;
  REAL bcd = pb[2] * cd - pc[2] * bd + pd[2] * bc;
  REAL cda = pc[2] * da + pd[2] * ac + pa[2] * cd;
  REAL dab = pd[2] * ab + pa[2] * bd + pb[2] * da;

  // weighted

  /*
  REAL alift = pa[0] * pa[0] + pa[1] * pa[1] + pa[2] * pa[2] - Sign(pa[3])*pa[3]*pa[3];
  REAL blift = pb[0] * pb[0] + pb[1] * pb[1] + pb[2] * pb[2] - Sign(pb[3])*pb[3]*pb[3];
  REAL clift = pc[0] * pc[0] + pc[1] * pc[1] + pc[2] * pc[2] - Sign(pc[3])*pc[3]*pc[3];
  REAL dlift = pd[0] * pd[0] + pd[1] * pd[1] + pd[2] * pd[2] - Sign(pd[3])*pd[3]*pd[3];
  */

  REAL alift = pa[0] * pa[0] + pa[1] * pa[1] + pa[2] * pa[2] - pa[3];
  REAL blift = pb[0] * pb[0] + pb[1] * pb[1] + pb[2] * pb[2] - pb[3];
  REAL clift = pc[0] * pc[0] + pc[1] * pc[1] + pc[2] * pc[2] - pc[3];
  REAL dlift = pd[0] * pd[0] + pd[1] * pd[1] + pd[2] * pd[2] - pd[3];

  REAL aliftplus = pa[0] * pa[0] + pa[1] * pa[1] + pa[2] * pa[2] + Abs(pa[3]);
  REAL bliftplus = pb[0] * pb[0] + pb[1] * pb[1] + pb[2] * pb[2] + Abs(pb[3]);
  REAL cliftplus = pc[0] * pc[0] + pc[1] * pc[1] + pc[2] * pc[2] + Abs(pc[3]);
  REAL dliftplus = pd[0] * pd[0] + pd[1] * pd[1] + pd[2] * pd[2] + Abs(pd[3]);

  REAL det = (dlift * abc - clift * dab) + (blift * cda - alift * bcd);

  REAL aezplus = Abs(pa[2]);
  REAL bezplus = Abs(pb[2]);
  REAL cezplus = Abs(pc[2]);
  REAL dezplus = Abs(pd[2]);
  REAL aexbeyplus = Abs(aexbey);
  REAL bexaeyplus = Abs(bexaey);
  REAL bexceyplus = Abs(bexcey);
  REAL cexbeyplus = Abs(cexbey);
  REAL cexdeyplus = Abs(cexdey);
  REAL dexceyplus = Abs(dexcey);
  REAL dexaeyplus = Abs(dexaey);
  REAL aexdeyplus = Abs(aexdey);
  REAL aexceyplus = Abs(aexcey);
  REAL cexaeyplus = Abs(cexaey);
  REAL bexdeyplus = Abs(bexdey);
  REAL dexbeyplus = Abs(dexbey);
  REAL permanent = ((cexdeyplus + dexceyplus) * bezplus
            ...
            * (dliftplus);
  REAL errbound = isperrboundA * permanent;
  if ((det > errbound) || (-det > errbound)) {
    return det;
  }
