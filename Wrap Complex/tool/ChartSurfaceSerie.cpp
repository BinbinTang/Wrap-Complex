...
void CChartSurfaceSerie::DrawAll(CDC* pDC)
{
	...
	for (unsigned index=uFirst; index<=uLast; index++)
	{
		...
		ValueToScreen(Point.X_, Point.Y_, pPoints[index-uFirst+1]);
	}
...
{ 
	...
	for (unsigned ptIndex=uFirst; ptIndex<uLast-1; ptIndex++)
	{
		...
		CPoint point1;
		CPoint point2;
		ValueToScreen(SeriePoint1.X_, SeriePoint1.Y_, point1);
		ValueToScreen(SeriePoint2.X_, SeriePoint2.Y_, point2);
