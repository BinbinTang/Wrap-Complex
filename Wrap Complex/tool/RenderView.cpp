...
//    glMatrixMode(GL_MODELVIEW); 	OutputGlError("MatrixMode") ;
	...
	if(m_LeftButtonDown)
	{
		glRotated((180*(-m_LeftDownPos.x + m_CurrPos.x))/WinWidth,0,1,0);
		glRotated((180*(-m_LeftDownPos.y + m_CurrPos.y))/WinWidth,1,0,0);
	}
...
*/	
	// Start rendering...
	glScalef(m_Scaling*m_Scaling,m_Scaling*m_Scaling,m_Scaling*m_Scaling);
...
void CRenderView::OnTimer(UINT nIDEvent) 
{
	...
	case 1:
		...
		break;
	case 2:
		((CToolDoc *)GetDocument())->BreathMotion();
		InvalidateRect(NULL,FALSE);
		break;
	default:
		{}
