     ...
     /* Opens file with given path_name in given mode. Cf, man fopen. */
{
  ...
  /* fopen in VC++ 5 currently does not handle null and
     empty file names correctly, so put a check for this
  */
  if ((!path_name) || (strlen(path_name) < 1))
	  return NULL;

  //sprintf (fname, "%s", path_name);
  sprintf_s (fname, MAXPATHLEN, "%s", path_name);
  //sprintf (fmode, "%s", mode);
  sprintf_s (fmode, 20, "%s", mode);

  fopen_s(&file, fname, mode);
  return file;
  //return (file = fopen(fname, fmode));
}
     ...
     /* Opens file with given path_name in given mode. Cf, man fopen. */
{
  ...
  */
  ...
  { /* check for compressed file first */
    ...
    //sprintf (zname, frmt[scheme], fname);
	sprintf_s (zname, MAXPATHLEN, frmt[scheme], fname);
    if (    (scheme != 0)  /* in this case, zname == "" */
        and (_access (zname, F_OK) != -1))
      {
        if (strpbrk (fmode, "wW"))
          {
            if (_access (zname, W_OK) == -1)
              basic_error ("basic_fopen: Can't remove file \"%s\".",
                           zname);
            printf ("Removing compressed file \"%s\" ...\n", zname);
     ...
     /* Closes the given file.  Cf, man fclose. */
{
#ifndef AUTOMATIC_UNCOMPRESS
  if (fclose (file) != 0)
    fprintf (stderr, "WARNING: basic_fclose: Unsucessful\n");
#else
  Plist *p = plist;
  while (p != NULL)
    { /* if file is in plist, using pclose() */
      if (p->id == _fileno (file))
        { 
          if (pclose (file) != 0)
            fprintf (stderr, "WARNING: basic_fclose: Unsucessful pclose()\n");
  ...
  if (fclose (file) != 0)
    fprintf (stderr, "WARNING: basic_fclose: Unsucessful fclose()\n");
#endif
}
     ...
     /* Output: scheme */
{
  ...
  while (frmt[i])
    {
      //sprintf (zname, frmt[i], fname);
	  sprintf_s (zname, MAXPATHLEN, frmt[i], fname);
      if (_access (zname, F_OK) != -1)
        {
          *scheme = i;
#if 1
          printf ("(Using \"%s %s\" for decompression.)\n",
                 cmd[i], frmt[i]);
#endif    
          break;
        }
     ...
     /* Checks if file with given path_name exists. Cf, man access. */
{
  ...
  if (_access (path_name, F_OK) != -1)
    ...
    { /* check for path_name.Z */
      ...
      //sprintf (zname, frmt[scheme], path_name);
	  sprintf_s (zname, MAXPATHLEN, frmt[scheme], path_name);
      return (_access (zname, F_OK) != -1); 
    }
#endif
}
     ...
     /* Removes file with given path_name[]. */
{
  ...
  if (_unlink (path_name) != 0)
    fprintf (stderr, "WARNIG: basic_rm (\"%s\") failed!\n", path_name);
}
