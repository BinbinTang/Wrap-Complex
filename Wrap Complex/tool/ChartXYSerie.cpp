...
{
	m_vPoints[PointIndex].X_ = NewVal;
...
{
	m_vPoints[PointIndex].Y_ = NewVal;
	m_vPoints.ReorderPoints();
											   ...
											   SChartXYPoint* &pSecondControlPoints) const
{
	...
	int i=0;
	for (i = 1; i < Count - 1; ++i)
		rhs[i] = 4 * pKnots[i].X_ + 2 * pKnots[i + 1].X_;
	rhs[0] = pKnots[0].X_ + 2 * pKnots[1].X_;
	rhs[Count - 1] = 3 * pKnots[Count - 1].X_;
	// Get first control points X-values
	double* pFirstX = GetFirstControlPoints(rhs, Count);

	// Set right hand side Y values
	for (i = 1; i < Count - 1; ++i)
		rhs[i] = 4 * pKnots[i].Y_ + 2 * pKnots[i + 1].Y_;
	rhs[0] = pKnots[0].Y_ + 2 * pKnots[1].Y_;
	rhs[Count - 1] = 3 * pKnots[Count - 1].Y_;
	...
	{
		// First control point
		pFirstControlPoints[i].X_ = pFirstX[i];
		pFirstControlPoints[i].Y_ = pFirstY[i];
		// Second control point
		if (i < Count - 1)
		{
			pSecondControlPoints[i].X_ =  2 * pKnots[i + 1].X_ - pFirstX[i + 1];
			pSecondControlPoints[i].Y_ =  2 * pKnots[i + 1].Y_ - pFirstY[i + 1];
		}
		else
		{
			pSecondControlPoints[i].X_ =  (pKnots[Count].X_ + pFirstX[Count - 1]) / 2;
			pSecondControlPoints[i].Y_ =  (pKnots[Count].Y_ + pFirstY[Count - 1]) / 2;
		}
	}
