...
void CChartLineSerie::DrawAll(CDC *pDC)
{
	...
	if (m_bSmooth)
	{
		...
		{
			ValueToScreen(pKnots[n].X_, pKnots[n].Y_, pBezierPts[index]);
			ValueToScreen(pFirstControlPts[n].X_, pFirstControlPts[n].Y_, pBezierPts[index+1]);
			ValueToScreen(pSecondControlPts[n].X_, pSecondControlPts[n].Y_, pBezierPts[index+2]);
		...
		ValueToScreen(pKnots[Count-1].X_, pKnots[Count-1].Y_, pBezierPts[index]);
		if (m_bShadow)
		{
			pShadowPts[index] = pBezierPts[index];
			pShadowPts[index].Offset(m_iShadowDepth,m_iShadowDepth);
	...
	{	
		if (uLast-uFirst >= 1)
		{
			...
			for (m_uLastDrawnPoint=uFirst;m_uLastDrawnPoint<=uLast;m_uLastDrawnPoint++)
			{
				...
				CPoint ScreenPoint;
				ValueToScreen(Point.X_, Point.Y_, ScreenPoint);
...
bool CChartLineSerie::IsPointOnSerie(const CPoint& screenPoint, unsigned& uIndex) const
{
	...
	for (unsigned i=uFirst ; i < uLast ; i++)
	{
		...
		ValueToScreen(PointOrig.X_, PointOrig.Y_, ScreenPointOrig);
		ValueToScreen(PointDest.X_, PointDest.Y_, ScreenPointDest);
