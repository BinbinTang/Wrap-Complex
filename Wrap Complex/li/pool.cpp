        ...
        filled up, a new one is allocated. */
{
  ...
  p->block_digits = ((block_size>MIN_BLOCK_SIZE)?block_size:MIN_BLOCK_SIZE) * lia_common.max;
...
#ifdef DO_TYPECHECKING
  minp = (Pool *) ((minp < p)? minp : p);
  maxp = (Pool *) ((maxp > p)? maxp : p);
#endif
        ...
        minlen = lia_get_maximum (). */
{
  if (not longi)
  ...
  else
    {
      ...
      /*int high = Max(lia_high (longi), minlen + 1);*/
      int high = (lia_high (longi) > (minlen + 1))? lia_high (longi) : (minlen + 1);
      Lia_ptr r;
