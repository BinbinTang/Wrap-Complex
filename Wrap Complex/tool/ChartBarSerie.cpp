...
CRect CChartBarSerie::GetBarRectangle(unsigned uPointIndex, int minorOffset) const
{
	...
	if (m_bHorizontal)
	{
		if (bCalcOffset)
		{
			...
			barXEnd = m_pHorizontalAxis->ValueToScreen(dXOffset+point.X_);
		}
		else 
		{
			barXEnd = m_pHorizontalAxis->ValueToScreen(point.X_);
			if (!m_bAutoBaseLine)
		...
		barYStart = m_pVerticalAxis->ValueToScreen(point.Y_) + minorOffset;
		barYEnd = m_pVerticalAxis->ValueToScreen(point.Y_) + minorOffset + m_iBarWidth;
	}
	else
	{
		if (bCalcOffset)
		{
			...
			barYEnd = m_pVerticalAxis->ValueToScreen(dYOffset+point.Y_);
		}
		else 
		{
			barYEnd = m_pVerticalAxis->ValueToScreen(point.Y_);
			if (!m_bAutoBaseLine)
		...
		barXStart = m_pHorizontalAxis->ValueToScreen(point.X_) + minorOffset;
		barXEnd = m_pHorizontalAxis->ValueToScreen(point.X_) + minorOffset + m_iBarWidth;
	}
