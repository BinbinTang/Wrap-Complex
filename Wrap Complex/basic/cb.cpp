...
int basic_cb_addalinef (FILE *file)
	...
	APPENDS them to the buffer.  Any character for which isspace() is
...
char* basic_cb_getline (FILE *file)
     /* Reads a line of text from the given file and returns a pointer to
        the corresponding string.  -->  Cf, basic_cb_addalinef().
        NULL is returned when EOF is reached.
	Use STRDUP() if necesssary!*/
{
  char *result;
  int io;
